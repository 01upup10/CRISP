...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def _derive_coco_results(self, coco_eval, class_names=None):
        ...
        return results
class IncrementalYTVISEvaluator(DatasetEvaluator):
    """
    Evaluate AR for object proposals, AP for instance detection/segmentation, AP
    for keypoint detection outputs using COCO's metrics.
    See http://cocodataset.org/#detection-eval and
    ...
    def __init__(
        ...
        *,
        ...
        self.currentTask_vidIDs = None
        self.currentTask_catIDs = None
    ...
    def evaluate(self):
        ...
        self._results = OrderedDict()
        self._eval_predictions_incremental(predictions)
        # Copy so the caller can do whatever with results
        return copy.deepcopy(self._results)

    def _eval_predictions_incremental(self, predictions):
        """
        Evaluate predictions. Fill self._results with the metrics of the tasks.
        """
        self._logger.info("Preparing results for YTVIS format ...")
        ...
        if hasattr(self._metadata, "thing_dataset_id_to_contiguous_id"):
            ...
            all_contiguous_ids = list(dataset_id_to_contiguous_id.values())
            num_classes = len(all_contiguous_ids)
            # current_task_contiguous_ids = all_contiguous_ids[star_c_id:end_c_id]
            assert min(all_contiguous_ids) == 0 and max(all_contiguous_ids) == num_classes - 1
        ...
        coco_eval = (
            _incremental_evaluate_predictions_on_coco(
                self._ytvis_api,
                predictions,
                None,
                self.incremental,
                self.currentTask_vidIDs,
                self.currentTask_catIDs
            )
            if len(predictions) > 0
            else None  # cocoapi does not handle empty results very well
        )
        if self.incremental:
            inds = list(map(lambda x:x-1, self.currentTask_catIDs))
            class_names=self._metadata.get("thing_classes")
            class_names = [class_names[i] for i in inds]
        else:
            class_names=self._metadata.get("thing_classes")
        res = self._derive_coco_results(
            coco_eval, class_names
        )
        self._results["segm"] = res
    ...
    def _derive_coco_results(self, coco_eval, class_names=None):
        ...
        results.update({"AP-" + name: ap for name, ap in results_per_category})
        return results
class IncrementalYTVOSeval(YTVOSeval):
    def evaluate(self, incremental=False, currentTask_vidIDs=None, currentTask_catIDs=None):
        '''
        ...
        if incremental:
            p.catIds = list(np.unique(currentTask_catIDs))
        p.vidIds = list(np.unique(currentTask_vidIDs))
        self.params=p
        self._prepare()
...
def _incremental_evaluate_predictions_on_coco(
    coco_gt,
    coco_results,
    img_ids=None,
    # crisp incremental eval
    incremental=False,
    currentTask_vidIDs=None,
    currentTask_catIDs=None,
):
    """
    Evaluate the coco results using COCOEval API.
    """
    ...
    coco_dt = coco_gt.loadRes(coco_results)
    coco_eval = IncrementalYTVOSeval(coco_gt, coco_dt)
    # For COCO, the default max_dets_per_image is [1, 10, 100].
    max_dets_per_image = [1, 10, 100]  # Default from COCOEval
    coco_eval.params.maxDets = max_dets_per_image
    ...
    coco_eval.evaluate(incremental,currentTask_vidIDs, currentTask_catIDs)
    coco_eval.accumulate()
    coco_eval.summarize()
