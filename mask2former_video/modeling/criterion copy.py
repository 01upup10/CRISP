...
from utils.loss import focal_loss, focal_uce_loss, unbiased_cross_entropy_loss, focal_distillation_loss, \
                        L2_distillation_loss, knowledge_distillation_loss, unbiased_knowledge_distillation_loss
...
class VideoSetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks):
        ...
        if self.empty_weight.shape[0] != src_logits.shape[-1]:
                    num_new_class = src_logits.shape[-1] - self.empty_weight.shape[0]
                    self.empty_weight = F.pad(self.empty_weight, (0, num_new_class), value=1.) # padding to same dim use 0
    ...
    def __get_labels__(self, outputs, targets):
        ...
        return {'preds':src_logits.transpose(1, 2), 
                'target_classes':target_classes}
    def __get_masks__(self, outputs, targets):
        ...
        return {'preds':src_masks,
                'targets':target_masks}


class  VideoSetCriterion_incremental(nn.Module):
    """This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, outputs_old=None):
        ...
        if self.empty_weight.shape[0] != src_logits.shape[-1]:
                    num_new_class = src_logits.shape[-1] - self.empty_weight.shape[0]
                    self.empty_weight = F.pad(self.empty_weight, (0, num_new_class), value=1.) # padding to same dim use 0
        if self.empty_weight.device != src_logits.device:
    ...
    def loss_masks_softmax(self,  outputs, targets, indices, num_masks, outputs_old=None):
        """Compute the losses related to the masks: the focal loss and the dice loss.
        targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        assert "pred_masks" in outputs
    ...
    def forward(self, outputs, targets, outputs_old=None):
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                indices = self.matcher(aux_outputs, targets)
                for loss in self.losses:
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_masks, outputs_old["aux_outputs"][i])
                    l_dict = {k + f"_{i}": v for k, v in l_dict.items()}
                    losses.update(l_dict)
