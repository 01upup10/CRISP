...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        h = [hidden_dim] * (num_layers - 1) # [hidden_dim, hidden_dim, ...]
...
class IncrementalLinearClassifier(nn.Module):
    '''
    classes: list; The number of categories per task
    old_params: dict;{'class_embed_weight': (tensor[old_classes_num, channels])
                      'class_embed.bias': (tensor[old_classes_num])}
    '''
    def __init__(self, classes, norm_feat=False, channels=256, bias=True, device='cuda'):
        super().__init__()
        # assert (old_params != None and old_params['class_embed_weight'].shape[0] == sum(classes[:-1])), "ERROR old_params shape wrong--(continual/modeling/classifier.py>>IncrementalLinearClassifier)"
        self.cls = nn.ModuleList(
                [nn.Linear(channels, sum(classes)+1, bias=bias)])
        self.norm_feat = norm_feat
